...
const MagneticButton = ({ children, href }: { children: React.ReactNode; href: string }) => {
  ...
  return (
    <motion.div style={{ x, y }} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className="inline-block">
      <Link ref={ref} href={href} className="relative inline-flex items-center justify-center px-8 py-4 bg-amber-500 text-black font-semibold overflow-hidden group rounded-full">
...
const ScrollProgress = () => {
  ...
  return <motion.div className="fixed top-0 left-0 right-0 h-1 bg-amber-500 origin-left z-[200]" style={{ scaleX }} />;
};
...
export default function HeroAceternity() {
  ...
  return (
    <>
      <ScrollProgress />
      <MouseSpotlight />
      <section ref={containerRef} className="relative min-h-screen w-full bg-stone-900 overflow-hidden flex items-center justify-center">
        ...
        <motion.div style={{ y, opacity }} className="relative z-10 max-w-5xl mx-auto px-6 lg:px-16 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} className="mb-8">
            <span className="text-stone-400 text-sm tracking-[0.3em] uppercase">Fine Dining</span>
          </motion.div>
          ...
          <motion.h1 className="text-5xl sm:text-6xl lg:text-8xl font-bold leading-[0.9] mb-8 text-white" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.3 }}>
            <TextReveal delay={0.4}>Experience Culinary Excellence</TextReveal>
          </motion.h1>

          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.6 }} className="text-lg text-stone-400 max-w-2xl mx-auto mb-12">
            Authentic flavors crafted with passion and the finest ingredients.
          </motion.p>

          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.8 }}>
            <MagneticButton href="#reservations">Book a Table</MagneticButton>
          </motion.div>
        </motion.div>
